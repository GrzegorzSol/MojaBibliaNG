...
const UnicodeString GsDirect2DClass::csVersion = "0.6.4689.1395";
//---------------------------------------------------------------------------
...
*/
{
	...
	unsigned int uiWidth, uiHeight;
	IWICBitmap *tpIWICBitmap=NULL;
	ID2D1RenderTarget *tpID2D1RenderTarget=NULL;
...
*/
{
  ...
  UnicodeString ustrPathToSave;
	...
	if(TPath::GetExtension(ustrPathToSave) == ".gif") GUIDFormat = GUID_ContainerFormatGif;
...
*/
{
  ...
  ::GetClientRect(this->Handle, &rRect);
	...
	D2D1_RECT_F rectangle = D2D1::RectF(static_cast<float>(rRect.left) / this->_dpiScaleX, static_cast<float>(rRect.top) / this->_dpiScaleY,
																			static_cast<float>(iWidth) / this->_dpiScaleX, static_cast<float>(iHeight) / this->_dpiScaleY);
	...
	if(BeginPaint(this->Handle, &Ps))
	{
		...
		if(SUCCEEDED(hr) && !(this->pID2D1HwndRenderTarget->CheckWindowState() & D2D1_WINDOW_STATE_OCCLUDED))
		{
			...
			if(this->FIsDisplayText)
			{
				...
				//Do rysowania należy użyć metody DrawText interfejsu docelowego renderowania D2D.
				this->pID2D1HwndRenderTarget->DrawText(
					this->FTextWrite.c_str(),       //Tekst do napisania
					this->FTextWrite.Length(),    	//Długość tekstu w znakach.
					this->pIDWriteTextFormat,    		//Format tekstu.
					rectangle,       								//Region okna, w którym zostanie wyświetlony tekst.
					this->pID2D1SolidColorBrush1    //Pędzel, który zostanie użyty do napisania tekstu
				);
			}
